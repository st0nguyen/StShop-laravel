...
class BrandProduct extends Controller
{
    ...
    public function all_brand_product()
    {
        ...
        $manager_brand_product
            = view('admin.all_brand_product')->with('all_brand_product',
            $all_brand_product);
...
//        DB::table('tbl_brand')->where('brand_id',$brand_product_id)->update(['brand_status'=>1]);
        Session::put('message',
            'Không kích hoạt thương hiệu sản phẩm thành công');
...
//        $edit_brand_product = DB::table('tbl_brand')->where('brand_id',$brand_product_id)->get();
        ...
        $manager_brand_product
            = view('admin.edit_brand_product')->with('edit_brand_product',
            $edit_brand_product);
...
//        DB::table('tbl_brand')->where('brand_id',$brand_product_id)->update($data);
        brand::find($brand_product_id)->update([
    ...
    public function show_brand_home($brand_slug)
    {
        ...
        $brand_by_id = DB::table('tbl_product')
            ->join('tbl_brand', 'tbl_product.brand_id', '=',
                'tbl_brand.brand_id')
