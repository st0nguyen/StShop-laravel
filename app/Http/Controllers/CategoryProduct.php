...
class CategoryProduct extends Controller
{
    ...
    {
        $this->AuthLogin();
        $all_category_product = \App\categoryProduct::all();
        $manager_category_product
            = view('admin.all_category_product')->with('all_category_product',
...
//    	DB::table('tbl_category_product')->insert($data);
        ...
        $cp = \App\categoryProduct::create($atribute);
    ...
    {
        $this->AuthLogin();
        \App\categoryProduct::find($category_product_id)->update(['category_status' => 1]);
...
//            ->update(['category_status' => 1]);
    ...
    {
        $this->AuthLogin();
        \App\categoryProduct::find($category_product_id)->update(['category_status' => 0]);
...
//            ->where('category_id', $category_product_id)->get();
        $edit_category_product
            = \App\categoryProduct::findOrFail($category_product_id);
...
//        $data['category_desc'] = $request->category_product_desc;
//        DB::table('tbl_category_product')
//            ->where('category_id', $category_product_id)->update($data);
        $a = \App\categoryProduct::find($category_product_id);
        $a->category_name = $request->category_name;
        $a->category_slug = $request->category_slug;
        $a->category_desc = $request->category_desc;
    ...
    {
        $this->AuthLogin();
        \App\categoryProduct::find($category_product_id)->delete();
    ...
    public function show_category_home($slug_category_product)
    {
        ...
        $category_by_id = DB::table('tbl_product')
            ->join('tbl_category_product', 'tbl_product.category_id', '=',
                'tbl_category_product.category_id')
            ->where('tbl_category_product.category_slug',
                $slug_category_product)->get();
